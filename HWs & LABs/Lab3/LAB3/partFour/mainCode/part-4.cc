...
NS_LOG_COMPONENT_DEFINE ("Lab3_part4");
	
// declaring parameters for simulation
...
// adding parameters to command Line.
CommandLine cmd;
...
// enabling packet meta Data for animator
ns3::PacketMetadata::Enable();
std::string animFile = "lab3_part4.xml";

// setting Up nodes
NodeContainer nodes;
nodes.Create(num_Nodes);

// creating Connections between network nodes.
...
// Overiding Routing from Static(default) to AnotherRouting(Dijkstra).
...
// setting up IPs ...
// setting up metrics  -> Graph weights ...
...
// setting Down Part
//getting i which is the last node
//8 refers to last one since node indexes start at 0
Ptr<Node> node1=nodes.Get(8);
Ptr<Ipv4> ipv41=node1->GetObject<Ipv4>();

//since I is connected to B,C,D,F,G,H we should set Down
// these indexes : B -> 1 , C -> 2 , D -> 3 , F -> 4 , G -> 5 , H -> 6
...
p2p.EnableAsciiAll (ascii.CreateFileStream ("Lab3_part4.tr"));
p2p.EnablePcapAll ("Lab3_part4");
}

// setUp lookUp Table.
Ptr<OutputStreamWrapper> stream1 = Create<OutputStreamWrapper> ("Table", std::ios::out);
Ipv4GlobalRoutingHelper helper2;
helper2.PrintRoutingTableAllAt(Seconds(4.0),stream1);
