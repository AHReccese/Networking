...
CsmaNetDevice::TransmitStart (void)
{
  ...
  if (m_channel->GetState () != IDLE)
    {
      ...
      if (m_backoff.MaxRetriesReached ())
      ...
      else 
        {
          ...
          Time backoffTime = m_backoff.m_slotTime;  // wait till next slot
  ...
  else // the channel is free 
    {
      // we should generate a random number between 0 and 1
         Ptr<UniformRandomVariable> m_rng;
         m_rng = CreateObject<UniformRandomVariable> ();
         float p = m_rng->GetValue (0, 1);
     ...
     if (p < 0.3)
     {
      ...
      if (m_backoff.MaxRetriesReached ())
        { 
          //
          // Too ma3.0ny retries, abort transmission of packet
          //
          TransmitAbort ();
        } 
      else 
        {
          ...
          NS_LOG_LOGIC ("Collision, backing off for " << backoffTime.GetSeconds () << " sec");
      ...
      // set a backoff time we want to wait
